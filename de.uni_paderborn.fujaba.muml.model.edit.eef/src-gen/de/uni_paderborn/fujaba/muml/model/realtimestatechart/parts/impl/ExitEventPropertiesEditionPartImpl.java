 ...
 * 
 */
public class ExitEventPropertiesEditionPartImpl extends CompositePropertiesEditionPart implements ISWTPropertiesEditionPart, ExitEventPropertiesEditionPart {
	 ...
	 * 
	 */
	public ExitEventPropertiesEditionPartImpl(IPropertiesEditionComponent editionComponent) {
	...
	protected Composite createPropertiesGroup(Composite parent) {
		...
		propertiesGroup.setText(RealtimestatechartMessages.ExitEventPropertiesEditionPart_PropertiesGroupLabel);
	...
	protected Composite createKindEMFComboViewer(Composite parent) {
		SWTUtils.createPartLabel(parent, RealtimestatechartMessages.ExitEventPropertiesEditionPart_KindLabel, propertiesEditionComponent.isRequired(RealtimestatechartViewsRepository.ExitEvent.Properties.kind, RealtimestatechartViewsRepository.SWT_KIND));
		...
		kind.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				if (propertiesEditionComponent != null)
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ExitEventPropertiesEditionPartImpl.this, RealtimestatechartViewsRepository.ExitEvent.Properties.kind, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.SET, null, getKind()));
			}

		});
		kind.setID(RealtimestatechartViewsRepository.ExitEvent.Properties.kind);
		SWTUtils.createHelpButton(parent, propertiesEditionComponent.getHelpContent(RealtimestatechartViewsRepository.ExitEvent.Properties.kind, RealtimestatechartViewsRepository.SWT_KIND), null); //$NON-NLS-1$
		return parent;
	}
	...
	protected Composite createClockResetsAdvancedReferencesTable(Composite parent) {
		...
		this.clockResets.setHelpText(propertiesEditionComponent.getHelpContent(RealtimestatechartViewsRepository.ExitEvent.Properties.clockResets, RealtimestatechartViewsRepository.SWT_KIND));
		this.clockResets.createControls(parent);
		this.clockResets.addSelectionListener(new SelectionAdapter() {
			
			public void widgetSelected(SelectionEvent e) {
				if (e.item != null && e.item.getData() instanceof EObject) {
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ExitEventPropertiesEditionPartImpl.this, RealtimestatechartViewsRepository.ExitEvent.Properties.clockResets, PropertiesEditionEvent.CHANGE, PropertiesEditionEvent.SELECTION_CHANGED, null, e.item.getData()));
				}
			}
		...
		"clockResets", propertiesEditionComponent.getEditingContext().getAdapterFactory(), current.eResource()) {
			...
			public void process(IStructuredSelection selection) {
				for (Iterator<?> iter = selection.iterator(); iter.hasNext();) {
					...
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ExitEventPropertiesEditionPartImpl.this, RealtimestatechartViewsRepository.ExitEvent.Properties.clockResets,
	...
	protected void moveClockResets(EObject element, int oldIndex, int newIndex) {
		propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ExitEventPropertiesEditionPartImpl.this, RealtimestatechartViewsRepository.ExitEvent.Properties.clockResets, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.MOVE, element, newIndex));
		clockResets.refresh();
	}
	...
	protected void removeFromClockResets(EObject element) {
		propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(ExitEventPropertiesEditionPartImpl.this, RealtimestatechartViewsRepository.ExitEvent.Properties.clockResets, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.REMOVE, null, element));
		clockResets.refresh();
	}
	 ...
	 * {@inheritDoc}
	 * 
	 * @see de.uni_paderborn.fujaba.muml.model.realtimestatechart.parts.ExitEventPropertiesEditionPart#initKind(Object input, Enumerator current)
	 */
