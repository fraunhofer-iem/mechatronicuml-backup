 ...
 * 
 */
public class AsynchronousMessageEventPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public AsynchronousMessageEventPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject asynchronousMessageEvent, String editing_mode) {
		super(editingContext, asynchronousMessageEvent, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = RealtimestatechartViewsRepository.class;
		partKey = RealtimestatechartViewsRepository.AsynchronousMessageEvent.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final AsynchronousMessageEvent asynchronousMessageEvent = (AsynchronousMessageEvent)elt;
			final AsynchronousMessageEventPropertiesEditionPart basePart = (AsynchronousMessageEventPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(RealtimestatechartViewsRepository.AsynchronousMessageEvent.Properties.kind)) {
				basePart.initKind(EEFUtils.choiceOfValues(asynchronousMessageEvent, RealtimestatechartPackage.eINSTANCE.getEvent_Kind()), asynchronousMessageEvent.getKind());
			}
			// init filters
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		AsynchronousMessageEvent asynchronousMessageEvent = (AsynchronousMessageEvent)semanticObject;
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			AsynchronousMessageEventPropertiesEditionPart basePart = (AsynchronousMessageEventPropertiesEditionPart)editingPart;
