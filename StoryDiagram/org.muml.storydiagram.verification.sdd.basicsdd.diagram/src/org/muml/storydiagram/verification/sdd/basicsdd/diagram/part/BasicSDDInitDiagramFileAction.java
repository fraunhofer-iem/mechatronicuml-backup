 ...
 * @generated
 */
public class BasicSDDInitDiagramFileAction extends AbstractCreateDiagramFileCommand {
	...
	*/
		...
		*/
		...
		public Object execute(ExecutionEvent event) throws ExecutionException {
			...
			if (selectedResource instanceof IContainer) {
				...
				String fileName = BasicSDDDiagramEditorUtil.getUniqueFileName(filePath, "StoryDecisionDiagram",
						"storydecisiondiagram");
				// Create the new file
				ResourceSet resourceSet = new ResourceSetImpl();
				URI uri = URI.createPlatformResourceURI(filePath.append(fileName).toString(), true);
				Resource resource = resourceSet.createResource(uri);
				EObject model = BasicSDDFactory.eINSTANCE.createStoryDecisionDiagram();
				resource.getContents().add(model);
				try {
					resource.save(Collections.emptyMap());
				} catch (IOException e) {
					BasicSDDDiagramEditorPlugin.getInstance().logError("Could not create child element", e);
				}
			}
			return null;
		}
	...
	public String getUniqueFilename(String hint, String extension, IPath filePath) {
		return BasicSDDDiagramEditorUtil.getUniqueFileName(filePath, hint, extension);
	}
