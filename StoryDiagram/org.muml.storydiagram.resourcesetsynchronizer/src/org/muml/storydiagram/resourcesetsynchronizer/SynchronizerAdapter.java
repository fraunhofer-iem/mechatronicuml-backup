...
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.SynchronizationProtocolFactory;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.DisconnectClientRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.EObjectAddedToResourceRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.EObjectRemovedFromResourceRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.FeatureSetRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.FeatureUnsetRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.GetConnectedSynchronizerAdaptersRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.ObjectAddedToFeatureRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.ObjectRemovedFromFeatureRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.ObjectsAddedToFeatureRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.ObjectsRemovedFromFeatureRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.RegisterClientRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.ResourceAddedRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.ResourceUriChangedRequest;
import org.muml.storydiagram.resourcesetsynchronizer.protocols.synchronization.requests.SyncInitialResourceSetContentsRequest;

/**
 * The SynchronizerAdapter can be attached to a resource set and connect to other
 * SynchronizerAdapters via TCP to keep all resource sets synchronized. All changes made in one
 * resource set are propagated to the others. Note, that the SynchronizerAdapters do not care about
 * concurrent modifications. The user has to take care that a resource set is not modified by more
 * than one thread at the same time. Modifications of the resource set due to received changes from
 * other SynchronizerAdapters are executed asynchronously.
 * 
 * Note further that currently only two connected SynchronizerAdapters have been tested. In
 * addition, it is currently not possible to "merge" resources, i.e. when two resource sets are
 * connected by synchronizer adapters, they may not contain resources with the same URIs.
 * 
 * For communication, the SynchronizerAdapter relies on NET4J. Each SynchronzierAdapter opens a
 * server to receive change events.
 * 
 * @author Stephan Hildebrandt
 * 
 */
public class SynchronizerAdapter extends EContentAdapter {
    private static final Logger LOGGER = Logger.getLogger(SynchronizerAdapter.class);
    ...
    /*
     * The EObjectUUIDManager manages the UUID of all EObjects in this resource set.
     */
    ...
    /*
     * Maps UUIDs of connected adapters to protocols. These protocols are needed to send events to
     * other SynchronizerAdapters.
     */
    private final Map<String, SignalProtocol<?>> protocols;

    /*
     * Maps UUIDs of connected adapters to their addresses and ports (value is "[address]:[port]").
     */
    ...
    /*
     * The command executor can be used to execute modifications on the resource set in a
     * transaction.
     */
    ...
    /**
     * Create a new SynchronizerAdapter and attach it to the provided resourceSet. The
     * SynchronizerAdapter will listen on the specified port for incoming message.
     * 
     * @param resourceSet
     *            The resourceSet to which the adapter will be attached.
     * @param serverPort
     *            The port on which the adapter will listen for incoming messages.
     */
    public SynchronizerAdapter(final ResourceSet resourceSet, final int serverPort,
            final ISynchronizerCommandExecutor commandExecutor) {
        ...
        LOGGER.info("SynchronizerAdapter(resourceSet," + serverPort + ", commandExecutor)");
    ...
    private SignalProtocol<Object> createSynchronizationProtocol() {
        return (SignalProtocol<Object>) this.synchronizationProtocolContainer.getFactory(
                ServerProtocolFactory.PRODUCT_GROUP, SynchronizationProtocolFactory.SYNCHRONIZATION_PROTOCOL).create(
                null);
    }

    /**
     * Connect this synchronizer adapter to the designated target synchronizer adapter to
     * synchronize both resources.
     * 
     * 
     * @param address
    ...
    public void connect(final String address, final int port) throws Exception {
        ...
        LOGGER.info("connect(" + address + ", " + port + ")");
         ...
         */
        final Map<String, String> synchronizerAdapters = new GetConnectedSynchronizerAdaptersRequest(
                synchronizationProtocol, this).send();
    ...
    private SignalProtocol<Object> registerAtServer(final String remoteAddress) throws Exception {
        LOGGER.info("registerAtServer(" + remoteAddress + ")");
         ...
         */
        final String remoteAdapterUUID = new RegisterClientRequest(synchronizationProtocol, this, this.getServerPort())
                .send();
    ...
    /**
     * Shutdown the server of this synchronizer and remove it from its resource set.
     * 
     * @throws Exception
     */
    ...
    /**
     * Registers the specified client synchronizer adapter with this synchronizer adapter and opens
     * a connection to the other adapter's server port.
     * 
    ...
    public void registerClient(final String remoteAdapterUUID, final String remoteAddress, final int remotePort) {
        ...
        LOGGER.info("registerClient(" + remoteAdapterUUID + ", " + remoteAddress + ", " + remotePort + ")");
    ...
    public void notifyChanged(final Notification notification) {
        try {
            ...
            /*
             * Skip notifications if ignoreNotifications is set (in case this adapter received a
             * change event from another synchronizer adapter and the change is currently applied)
             * or if the change does not change anything.
             */
                ...
                for (final Entry<String, SignalProtocol<?>> entry : this.protocols.entrySet()) {
                    ...
                    if (notification.getNotifier() instanceof ResourceSet) {
                        ...
                        case Notification.ADD: {
                            ...
                            new ResourceAddedRequest(entry.getValue(), this, (Resource) notification.getNewValue())
                                    .send();
                        ...
                        case Notification.REMOVE: {
                            ...
                            System.out.println("Resource has been removed: "
                                    + ((Resource) notification.getNewValue()).getURI());
                        ...
                        case Resource.RESOURCE__CONTENTS: {
                            ...
                            switch (notification.getEventType()) {
                            case Notification.ADD:
                                new EObjectAddedToResourceRequest(entry.getValue(), this,
                                        (EObject) notification.getNewValue(), notification.getPosition()).send();

                                break;
                            case Notification.ADD_MANY:
                                System.out.println("Objects were added to resource "
                                        + ((Resource) notification.getNotifier()).getURI() + ": "
                            ...
                            case Notification.REMOVE:
                                new EObjectRemovedFromResourceRequest(entry.getValue(), this,
                                        (Resource) notification.getNotifier(), (EObject) notification.getOldValue())
                                        .send();
                        ...
                        if (feature.isChangeable() && !feature.isDerived() && !feature.isTransient()) {
                            switch (notification.getEventType()) {
                            case Notification.ADD: {
                                new ObjectAddedToFeatureRequest(entry.getValue(), this,
                            ...
                            case Notification.ADD_MANY: {
                                new ObjectsAddedToFeatureRequest(entry.getValue(), this,
                                        (EObject) notification.getNotifier(),
                                        (EStructuralFeature) notification.getFeature(),
                                        (List<Object>) notification.getNewValue(), notification.getPosition()).send();

                                break;
                            }

                            case Notification.REMOVE: {
                                new ObjectRemovedFromFeatureRequest(entry.getValue(), this,
                                        (EObject) notification.getNotifier(),
                                        (EStructuralFeature) notification.getFeature(), notification.getOldValue())
                                        .send();
                            ...
                            case Notification.REMOVE_MANY: {
                                new ObjectsRemovedFromFeatureRequest(entry.getValue(), this,
                                        (EObject) notification.getNotifier(),
                                        (EStructuralFeature) notification.getFeature(),
                                        (List<Object>) notification.getOldValue()).send();
    ...
    public void disconnect(final String remoteAdapterUUID) {
        LOGGER.info("disconnect(" + remoteAdapterUUID + ")");
