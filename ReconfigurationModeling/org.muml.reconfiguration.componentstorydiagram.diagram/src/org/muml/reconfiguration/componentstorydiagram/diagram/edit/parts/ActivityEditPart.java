...
import org.muml.reconfiguration.componentstorydiagram.diagram.edit.policies.ActivityItemSemanticEditPolicy;
	...
	*/
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof ActivityNameEditPart) {
			((ActivityNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureActivityName());
			return true;
		}
		if (childEditPart instanceof ActivityActivityCompartmentEditPart) {
			IFigure pane = getPrimaryShape().getFigureActivityContainer();
			setupContentPane(pane); // FIXME each comparment should handle his content pane in his own way 
			pane.add(((ActivityActivityCompartmentEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof ActivityActivityCompartmentEditPart) {
			IFigure pane = getPrimaryShape().getFigureActivityContainer();
			pane.remove(((ActivityActivityCompartmentEditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
	...
	protected IFigure getContentPaneFor(IGraphicalEditPart editPart) {
		if (editPart instanceof ActivityActivityCompartmentEditPart) {
	...
	public EditPart getPrimaryChildEditPart() {
		return getChildBySemanticHint(ComponentStoryDiagramVisualIDRegistry.getType(ActivityNameEditPart.VISUAL_ID));
	}
	...
	public class ActivityFigure extends RectangleFigure {
		...
		private void createContents() {
			...
			polyline0.addPoint(new Point(getMapMode().DPtoLP(0), getMapMode().DPtoLP(2)));
			polyline0.addPoint(new Point(getMapMode().DPtoLP(20), getMapMode().DPtoLP(2)));
			polyline0.addPoint(new Point(getMapMode().DPtoLP(21), getMapMode().DPtoLP(1)));
			polyline0.addPoint(new Point(getMapMode().DPtoLP(21), getMapMode().DPtoLP(0)));
			polyline0.addPoint(new Point(getMapMode().DPtoLP(21), getMapMode().DPtoLP(1)));
			polyline0.addPoint(new Point(getMapMode().DPtoLP(20), getMapMode().DPtoLP(2)));
			polyline0.setFill(true);
			polyline0.setBorder(new MarginBorder(getMapMode().DPtoLP(2), getMapMode().DPtoLP(2), getMapMode().DPtoLP(2),
			...
			this.add(fFigureActivityContainer, constraintFFigureActivityContainer);
