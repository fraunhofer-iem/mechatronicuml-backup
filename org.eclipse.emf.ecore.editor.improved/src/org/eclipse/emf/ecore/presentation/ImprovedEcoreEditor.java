...
public class ImprovedEcoreEditor extends
	...
	protected ViewerSorter sorter = new ViewerSorter() {
		...
		private int getEClassNumber(EClass eClass) {
			EClass array[] = { EcorePackage.Literals.EANNOTATION, EcorePackage.Literals.EGENERIC_TYPE, EcorePackage.Literals.ESTRUCTURAL_FEATURE };
	...
	public ImprovedEcoreEditor() {
		inheritanceContentProvider = new InheritanceContentProvider(adapterFactory);
		
		EcoreEditorPlugin.getPlugin().getDialogSettings().put("showGenericsAction", Boolean.TRUE); 
	}
	...
	protected void doubleClicked(Object object) {
		...
		selectionViewer.setExpandedState(object, !selectionViewer.getExpandedState(object));
	}
	...
	protected Composite createPageContainer(final Composite parent) {
		...
		searchbox.setForeground(parent.getDisplay().getSystemColor(SWT.COLOR_GRAY));
		searchbox.addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent e) {
				if (DEFAULT_SEARCH_TEXT.equals(searchbox.getText())) {
					searchbox.setText("");
					searchbox.setForeground(parent.getDisplay().getSystemColor(SWT.COLOR_BLACK));
				}
			}
	...
	protected void setFilter(String filterText) {
	...
	protected void initializeToolBarManager(ToolBarManager toolBarManager) {
		...
		// Mark derived
		actionMarkDerived = new Action("Mark derived features", Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		// Inheritance: NONE
		actionInheritanceNone = new Action("No additional inheritance", Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
				if (isChecked()) {
		...
		actionInheritanceNone.setImageDescriptor(ImprovedEcoreEditorPlugin.getImageDescriptor("icons/none.gif"));
		
		// Inheritance: FEATURES
		actionInheritanceFeatures = new Action("Show inherited features", Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
				if (isChecked()) {
		...
		actionInheritanceFeatures.setImageDescriptor(ImprovedEcoreEditorPlugin.getImageDescriptor("icons/features.gif"));

		// Inheritance: HIERARCHY
		actionInheritanceHierarchy = new Action("Show supertype hierarchy", Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
				if (isChecked()) {
		...
		actionInheritanceHierarchy.setImageDescriptor(ImprovedEcoreEditorPlugin.getImageDescriptor("icons/hierarchy.gif"));
		
		// Expand all
		Action actionExpandAll = new Action("Expand All", ImprovedEcoreEditorPlugin.getImageDescriptor("icons/expandall.gif")) {
			@Override
			public void run() {
		...
		// Collapse all
		Action actionCollapseAll = new Action("Collapse All", ImprovedEcoreEditorPlugin.getImageDescriptor("icons/collapseall.gif")) {
			@Override
			public void run() {
		...
		// Read preferences
		activateSorter(Boolean.parseBoolean(ImprovedEcoreEditorPlugin.getPreferencesValue("sort", Boolean.TRUE.toString())));
		setMarkDerived(Boolean.parseBoolean(ImprovedEcoreEditorPlugin.getPreferencesValue("markDerived", Boolean.TRUE.toString())));
		setInheritanceMode(InheritanceMode.valueOf(ImprovedEcoreEditorPlugin.getPreferencesValue("inheritanceMode", InheritanceMode.NONE.toString())));
	...
	private void setInheritanceMode(InheritanceMode mode) {
		...
		ImprovedEcoreEditorPlugin.setPreferencesValue("inheritanceMode", mode.toString());
	}
	...
	public class ImprovedLabelProvider extends DefaultLabelProvider {
		...
		public ImprovedLabelProvider(AdapterFactory adapterFactory, Viewer viewer) {
			...
			colorInherited = defaultFont.getDevice().getSystemColor(SWT.COLOR_DARK_GRAY);
			
			ImageDescriptor descriptor = ImprovedEcoreEditorPlugin.getImageDescriptor("icons/supertype.gif");
			if  (descriptor != null) {
		...
		public String getText(Object element) {
			...
			if (element instanceof InheritanceNode) {
				...
				if (node.object instanceof EStructuralFeature && node.mode == InheritanceMode.FEATURES) {
		...
		public Image getImage(Object object) {
			if (object instanceof InheritanceNode) {
				...
				if (node.object instanceof EClass && node.mode == InheritanceMode.HIERARCHY) {
	...
	public void setMarkDerived(boolean markDerived) {
		...
		ImprovedEcoreEditorPlugin.setPreferencesValue("markDerived", Boolean.valueOf(markDerived).toString());
	}
	...
	public void activateSorter(boolean active) {
		...
		// Set preference
		ImprovedEcoreEditorPlugin.setPreferencesValue("sort", Boolean.valueOf(active).toString());
	}
