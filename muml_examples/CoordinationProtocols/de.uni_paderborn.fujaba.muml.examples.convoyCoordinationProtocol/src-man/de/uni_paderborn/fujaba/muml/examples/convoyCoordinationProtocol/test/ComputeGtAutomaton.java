package de.uni_paderborn.fujaba.muml.examples.convoyCoordinationProtocol.test;
...
public class ComputeGtAutomaton {
	...
	private final String modelURI = "/de.uni_paderborn.fujaba.muml.examples.convoyCoordinationProtocol/model/ConvoyCoordinationProtocol.fujaba";
			...
			if(cat.getKey().equals("de.uni_paderborn.fujaba.muml.protocol.category")){
				for(EObject curObj : cat.getModelElements()){
					if(curObj instanceof CoordinationProtocol){
						...
						if(curProt.getName().equals("ProfileTransmission")){
							return curProt;
						}
					}
				}
	...
	public void computeGtAutomaton() throws IOException{
		...
		GtAutomatonComputation reachComp = new GtAutomatonComputation(initialGraphObjects, tgts);
