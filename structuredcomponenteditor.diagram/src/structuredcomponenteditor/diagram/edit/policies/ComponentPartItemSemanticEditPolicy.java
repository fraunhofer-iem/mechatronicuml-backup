...
import structuredcomponenteditor.diagram.edit.commands.Port2CreateCommand;
 ...
 * @generated
 */
public class ComponentPartItemSemanticEditPolicy extends
	...
	public ComponentPartItemSemanticEditPolicy() {
		super(StructuredcomponentElementTypes.ComponentPart_3002);
	}
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		if (StructuredcomponentElementTypes.Port_3003 == req.getElementType()) {
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (StructuredcomponentVisualIDRegistry.getVisualID(node)) {
			case Port2EditPart.VISUAL_ID:
				for (Iterator<?> it = node.getTargetEdges().iterator(); it
						.hasNext();) {
					Edge incomingLink = (Edge) it.next();
					if (StructuredcomponentVisualIDRegistry
							.getVisualID(incomingLink) == DelegationEditPart.VISUAL_ID) {
					...
					if (StructuredcomponentVisualIDRegistry
							.getVisualID(incomingLink) == AssemblyEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
								incomingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(),
				...
				for (Iterator<?> it = node.getSourceEdges().iterator(); it
					...
					if (StructuredcomponentVisualIDRegistry
							.getVisualID(outgoingLink) == DelegationEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
					...
					if (StructuredcomponentVisualIDRegistry
							.getVisualID(outgoingLink) == AssemblyEditPart.VISUAL_ID) {
						DestroyElementRequest r = new DestroyElementRequest(
								outgoingLink.getElement(), false);
						cmd.add(new DestroyElementCommand(r));
						cmd.add(new DeleteCommand(getEditingDomain(),
