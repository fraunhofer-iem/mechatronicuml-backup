...
import structuredcomponenteditor.diagram.part.StructuredcomponentDiagramUpdater;
import structuredcomponenteditor.diagram.part.StructuredcomponentLinkDescriptor;
import structuredcomponenteditor.diagram.part.StructuredcomponentNodeDescriptor;
...
public class StructuredComponentDiagramCanonicalEditPolicy extends
	...
	protected List getSemanticChildrenList() {
		...
		List<StructuredcomponentNodeDescriptor> childDescriptors = StructuredcomponentDiagramUpdater
				.getStructuredComponentDiagram_1000SemanticChildren(viewObject);
		for (StructuredcomponentNodeDescriptor d : childDescriptors) {
	...
	private boolean isMyDiagramElement(View view) {
		return StructuredComponentEditPart.VISUAL_ID == StructuredcomponentVisualIDRegistry
				.getVisualID(view);
	}
	...
	protected void refreshSemantic() {
		...
		for (Iterator<StructuredcomponentNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			StructuredcomponentNodeDescriptor next = descriptorsIterator.next();
			String hint = StructuredcomponentVisualIDRegistry.getType(next
					.getVisualID());
		...
		for (StructuredcomponentNodeDescriptor next : childDescriptors) {
			String hint = StructuredcomponentVisualIDRegistry.getType(next
					.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<StructuredcomponentLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = StructuredcomponentVisualIDRegistry
			...
			for (Iterator<StructuredcomponentLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				StructuredcomponentLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<StructuredcomponentLinkDescriptor> collectAllLinks(
		...
		case StructuredComponentDiagramEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(StructuredcomponentDiagramUpdater
	...
	private Collection<IAdaptable> createConnections(
			Collection<StructuredcomponentLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (StructuredcomponentLinkDescriptor nextLinkDescriptor : linkDescriptors) {
