...
import org.eclipse.emf.ecore.parts.EcoreViewsRepository.EStringToStringMapEntry;
...
public class EAnnotationPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (eAnnotation.getSource() != null && isAccessible(EcoreViewsRepository.EAnnotation.Properties.source))
				basePart.setSource(EEFConverterUtil.convertToString(EcorePackage.eINSTANCE.getEString(), eAnnotation.getSource()));
			...
			basePart.addFilterToDetails(new ViewerFilter() {
					...
					public boolean select(Viewer viewer, Object parentElement, Object element) {
						return (element instanceof String && element.equals("")) || (element instanceof EStringToStringMapEntry); //$NON-NLS-1$ 
					}
			...
			basePart.addFilterToReferences(new ViewerFilter() {
				...
				public boolean select(Viewer viewer, Object parentElement, Object element) {
					if (element instanceof EObject)
						return (!basePart.isContainedInReferencesTable((EObject)element));
					return element instanceof Resource;
				}
			
			});
			basePart.addFilterToReferences(new EObjectFilter(EcorePackage.eINSTANCE.getEObject()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (EcoreViewsRepository.EAnnotation.Properties.source == event.getAffectedEditor()) {
			eAnnotation.setSource((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.eINSTANCE.getEString(), (String)event.getNewValue()));
		}
		if (EcoreViewsRepository.EAnnotation.Properties.details == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				...
				detailsSettings.move(event.getNewIndex(), (EStringToStringMapEntryImpl) event.getNewValue());
			}
		}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			...
			if (EcorePackage.eINSTANCE.getEAnnotation_Source().equals(msg.getFeature()) && basePart != null && isAccessible(EcoreViewsRepository.EAnnotation.Properties.source)) {
  ...
  public boolean mustBeComposed(Object key, int kind) {
    return key == EcoreViewsRepository.EAnnotation.Properties.source || key == EcoreViewsRepository.EAnnotation.Properties.eModelElement || key == EcoreViewsRepository.EAnnotation.Properties.contents || key == EcoreViewsRepository.EAnnotation.Properties.references || key == EcoreViewsRepository.EAnnotation.Properties.class;
  }
			...
			try {
				if (EcoreViewsRepository.EAnnotation.Properties.source == event.getAffectedEditor()) {
					...
					if (newValue instanceof String) {
						newValue = EcoreUtil.createFromString(EcorePackage.eINSTANCE.getEAnnotation_Source().getEAttributeType(), (String)newValue);
					}
