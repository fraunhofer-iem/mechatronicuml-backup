package de.uni_paderborn.fujaba.muml.hardware.platform.diagram.custom.providers;

import org.eclipse.gmf.runtime.common.ui.services.parser.IParser;

import de.uni_paderborn.fujaba.muml.hardware.platform.diagram.custom.parsers.CustomBusExpressionLabelParser5023;
import de.uni_paderborn.fujaba.muml.hardware.platform.diagram.custom.parsers.CustomHWPlatformPartPartCardinalityExpressionLabelParser5028;
import de.uni_paderborn.fujaba.muml.hardware.platform.diagram.custom.parsers.CustomLinkExpressionLabelParser6002;
import de.uni_paderborn.fujaba.muml.hardware.platform.diagram.custom.parsers.CustomResourceInstancePartCardinalityExpressionLabelParser5032;
import de.uni_paderborn.fujaba.muml.hardware.platform.diagram.providers.HardwareParserProvider;
/**
 * Our custom ParserProvider, which uses the customized LabelParsers.
 * 
 * @author adann
 * 
 */
public class CustomPlatformParserProvider extends HardwareParserProvider {
	protected IParser getParser(int visualID) {
		switch (visualID) {
		case de.uni_paderborn.fujaba.muml.hardware.platform.diagram.edit.parts.WrappingLabel7EditPart.VISUAL_ID:
			return new CustomResourceInstancePartCardinalityExpressionLabelParser5032();
			
		case de.uni_paderborn.fujaba.muml.hardware.platform.diagram.edit.parts.WrappingLabel5EditPart.VISUAL_ID:
			return new CustomHWPlatformPartPartCardinalityExpressionLabelParser5028();
			
		case de.uni_paderborn.fujaba.muml.hardware.platform.diagram.edit.parts.WrappingLabelEditPart.VISUAL_ID:
			return new CustomBusExpressionLabelParser5023();
			
		case de.uni_paderborn.fujaba.muml.hardware.platform.diagram.edit.parts.WrappingLabel8EditPart.VISUAL_ID:
			return new CustomLinkExpressionLabelParser6002();
		
		case de.uni_paderborn.fujaba.muml.hardware.platform.diagram.edit.parts.WrappingLabel9EditPart.VISUAL_ID:
			return new CustomLinkExpressionLabelParser6002();
			
		
		
		} 
		return super.getParser(visualID);
	}


}
