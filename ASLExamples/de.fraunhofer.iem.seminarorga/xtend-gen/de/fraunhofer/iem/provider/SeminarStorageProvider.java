...
public class SeminarStorageProvider extends EObjectImpl implements StorageProvider {
  ...
  protected Seminar _storePair(final Student source, final Topic target) {
    try {
      ...
      {
        final Assignment assignment = SeminarOrgaFactory.eINSTANCE.createAssignment();
        final EditingDomain editingDomain = AdapterFactoryEditingDomain.getEditingDomainFor(this.seminar);
        assignment.setStudent(source);
        assignment.setTopic(target);
        final Command addCommand = AddCommand.create(editingDomain, this.seminar, this.seminar.getAssignments(), assignment);
