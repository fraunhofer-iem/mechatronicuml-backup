...
import patterneditor.diagram.part.PatternDiagramUpdater;
import patterneditor.diagram.part.PatternLinkDescriptor;
import patterneditor.diagram.part.PatternNodeDescriptor;
...
public class PatternDiagramCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize.add(PatterneditorPackage.eINSTANCE
	...
	private boolean isMyDiagramElement(View view) {
		int visualID = PatternVisualIDRegistry.getVisualID(view);
		return visualID == CoordinationPatternEditPart.VISUAL_ID
				|| visualID == RoleEditPart.VISUAL_ID
				|| visualID == TextualConstraintEditPart.VISUAL_ID;
	}
	...
	protected void refreshSemantic() {
		...
		for (Iterator<PatternNodeDescriptor> descriptorsIterator = childDescriptors
				.iterator(); descriptorsIterator.hasNext();) {
			PatternNodeDescriptor next = descriptorsIterator.next();
			String hint = PatternVisualIDRegistry.getType(next.getVisualID());
		...
		for (PatternNodeDescriptor next : childDescriptors) {
			String hint = PatternVisualIDRegistry.getType(next.getVisualID());
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<PatternLinkDescriptor> linkDescriptors = collectAllLinks(
				getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
				.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = PatternVisualIDRegistry
			...
			for (Iterator<PatternLinkDescriptor> linkDescriptorsIterator = linkDescriptors
					.iterator(); linkDescriptorsIterator.hasNext();) {
				PatternLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	 ...
	 * @generated
	 */
	private Collection<PatternLinkDescriptor> collectAllLinks(View view,
			Map<EObject, View> domain2NotationMap) {
		if (!PatternDiagramEditPart.MODEL_ID.equals(PatternVisualIDRegistry
				.getModelID(view))) {
	...
	private Collection<IAdaptable> createConnections(
			Collection<PatternLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
		LinkedList<IAdaptable> adapters = new LinkedList<IAdaptable>();
		for (PatternLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					PatternVisualIDRegistry.getType(nextLinkDescriptor
