...
public class TGGTransformation {
	...
	public SimulinkContainer transform(RootNode source, IProgressMonitor monitor)
	{
		...
		try{
			tgg = InterpreterconfigurationUtil.loadAndInitTripleGraphGrammar(URI.createPlatformResourceURI("/de.uni_paderborn.fujaba2simulink/tgg/fujaba2simulink.tgg",true), resourceSet);
		}
		// else
		catch(Exception e){
			tgg = InterpreterconfigurationUtil.loadAndInitTripleGraphGrammar(URI.createPlatformPluginURI("de.uni_paderborn.fujaba.simulink.wizard/tgg/fujaba2simulink.tgg",true), resourceSet);
		}
	...
	private Configuration buildConfiguration(TripleGraphGrammar tgg, RootNode source)
	{
		//1. Set TGG
