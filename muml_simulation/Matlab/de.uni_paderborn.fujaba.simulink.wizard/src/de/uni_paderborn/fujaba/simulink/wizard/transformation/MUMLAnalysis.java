package de.uni_paderborn.fujaba.simulink.wizard.transformation;
...
import de.uni_paderborn.fujaba.muml.realtimestatechart.impl.RealtimestatechartFactoryImpl;
import de.uni_paderborn.fujaba.muml.realtimestatechart.impl.SynchronizationChannelImpl;
...
//								stateChartContainsStateChart(rtsc, (RealtimeStatechart) dp.getBehavior());						
	...
	public void createSyncVariables(ArrayList<Transition> statechart_receivers){
		...
		for(Transition trans : statechart_receivers){
			...
			State targetState = (State)trans.getTarget();
		...
		for(Transition trans : statechart_receivers){
			if(trans.getSynchronization().getSelectorExpression() != null){
				String condition = "selCond" + trans.getStatechart().getParentRegion().getName().substring(0, 1).toUpperCase()
									 ...
									 + ((State)trans.getSource()).getName().substring(0, 1).toUpperCase()
									 + ((State)trans.getSource()).getName().substring(1, ((State)trans.getSource()).getName().length())
									 + ((State)trans.getTarget()).getName().substring(0, 1).toUpperCase()
									 + ((State)trans.getTarget()).getName().substring(1, ((State)trans.getTarget()).getName().length())
									 + trans.getPriority() + " = ";
			...
			for(Region reg : regions.keySet()){
				...
				for(Transition recTrans : regions.get(reg)){
					if(recTrans.getSynchronization().getSyncChannel().equals(channel)){
						...
						State targetState = (State)recTrans.getTarget();
						...
						if(hasSelectorType){
							selectorCondition = "(selCond" + recTrans.getStatechart().getParentRegion().getName().substring(0, 1).toUpperCase()
								       ...
								       + ((State)recTrans.getSource()).getName().substring(0, 1).toUpperCase()
								       + ((State)recTrans.getSource()).getName().substring(1, ((State)recTrans.getSource()).getName().length())
								       + ((State)recTrans.getTarget()).getName().substring(0, 1).toUpperCase()
								       + ((State)recTrans.getTarget()).getName().substring(1, ((State)recTrans.getTarget()).getName().length())
								       + recTrans.getPriority()
								       + " == (" + ownSelCond + "))";
