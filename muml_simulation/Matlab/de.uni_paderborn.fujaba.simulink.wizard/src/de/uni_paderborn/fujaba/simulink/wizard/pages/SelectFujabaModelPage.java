package de.uni_paderborn.fujaba.simulink.wizard.pages;
...
import de.uni_paderborn.fujaba.simulink.wizard.NameAnalysis;
import de.uni_paderborn.fujaba.simulink.wizard.TransformationWizard;
import de.uni_paderborn.fujaba.simulink.wizard.editors.FujabaSelectionEditor;
	...
	{	
		...
		if(MUMLAnalysis.modelContainsArrayParameters(this.model))
		{
			if(!MessageDialog.openQuestion(getShell(), "Model contains message types with ArrayDataType-parameters", 
					"Transformation of ArrayDataType-parameters is not supported in the current version. Should the model be transformed nevertheless?"))
				{
					this.getShell().close();
					return;
				}			
		...
		if(MUMLAnalysis.modelContainsArrayVariables(this.model))
		{
			if(!MessageDialog.openQuestion(getShell(), "Model contains ArrayDataType-attributes", 
					"Transformation of ArrayDataType-attributes is not supported in the current version. Should the model be transformed nevertheless?"))
				{
					this.getShell().close();
					return;
				}			
