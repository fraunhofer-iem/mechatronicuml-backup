package de.uni_paderborn.fujaba.graphviz.xtext.tests
...
import de.uni_paderborn.fujaba.graphviz.xtext.PlainInjectorProvider


@RunWith(typeof(XtextRunner))
@InjectWith(typeof(PlainInjectorProvider))
class PlainTest {
	@Inject
	ParseHelper<Graph> parser
...
stop
  		...
  		assertNotNull(graph);
  		assertEquals(1, graph.nodes.size());
  		assertEquals("\"PositionTransmission sender\"", graph.nodes.get(0).name);
  	}
}