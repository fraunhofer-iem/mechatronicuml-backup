...
void M1MyMessageM1Message_clear(struct M1MyMessageM1Message *_M1MyMessageM1Message) {
	_memset(_M1MyMessageM1Message, 0, sizeof(struct M1MyMessageM1Message));
}

void M1MyMessageM1Message_init_optional_attributes(struct M1MyMessageM1Message *_M1MyMessageM1Message) {
	_M1MyMessageM1Message->_lightValue = 0.0;
	
}

int M1MyMessageM1Message_is_default_message(struct M1MyMessageM1Message *_M1MyMessageM1Message) {
    return _M1MyMessageM1Message->_lightValue == 0.0
    ;
}

int M1MyMessageM1Message_write(struct M1MyMessageM1Message *_M1MyMessageM1Message, void *_buffer, int offset) {
	/* Write content of each message element.*/
	/* Write the optional attribute only if it is different than the default value. */
	if(_M1MyMessageM1Message->_lightValue != 0) {
		offset = write_raw_varint32((1<<3)+1, _buffer, offset);
	    unsigned long long*p1_ptr = (unsigned long long*)&_M1MyMessageM1Message->_lightValue;
	    offset = write_raw_little_endian64(*p1_ptr, _buffer, offset);
	}
	
	return offset;
}

int M1MyMessageM1Message_write_with_tag(struct M1MyMessageM1Message *_M1MyMessageM1Message, void *_buffer, int offset, int tag) {
	/* Write tag.*/
	offset = write_raw_varint32((tag<<3)+2, _buffer, offset);
	/* Write content.*/
	offset = M1MyMessageM1Message_write_delimited_to(_M1MyMessageM1Message, _buffer, offset);
	
	return offset;
}

int M1MyMessageM1Message_write_delimited_to(struct M1MyMessageM1Message *_M1MyMessageM1Message, void *_buffer, int offset) {
	int i, shift, new_offset, size;
	
	new_offset = M1MyMessageM1Message_write(_M1MyMessageM1Message, _buffer, offset);
...
int M1MyMessageM1Message_read(void *_buffer, struct M1MyMessageM1Message *_M1MyMessageM1Message, int offset, int limit) {
	unsigned int i = 0;
	unsigned long long value = i;
	unsigned long tag = value;
	
	/* Reset all attributes to 0 in advance. */
	M1MyMessageM1Message_clear(_M1MyMessageM1Message);
	/* Assign the optional attributes. */
	M1MyMessageM1Message_init_optional_attributes(_M1MyMessageM1Message);
	...
	while(offset < limit) {
		...
		tag = tag>>3;
	    switch(tag){
	        /* tag of: _M1MyMessageM1Message._p1 */
	        case 1 :
	        	offset = read_raw_little_endian64(&value, _buffer, offset);
	        	double *lightValue = (double *)(&value);
	        	_M1MyMessageM1Message->_lightValue = *lightValue;
	        	break;
	    }
	}
...
int M1MyMessageM1Message_read_delimited_from(void *_buffer, struct M1MyMessageM1Message *_M1MyMessageM1Message, int offset) {
	unsigned long size;
	
	offset = read_raw_varint32(&size, _buffer, offset);
	M1MyMessageM1Message_read(_buffer, _M1MyMessageM1Message, offset, size + offset);
