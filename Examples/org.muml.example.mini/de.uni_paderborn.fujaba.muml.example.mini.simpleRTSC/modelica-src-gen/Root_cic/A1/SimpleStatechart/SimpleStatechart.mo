...
model SimpleStatechart
	annotation (Diagram(coordinateSystem(extent = {{-25.4, -12.7}, {25.4, 12.7}})));
		...
		// step declarations
		Complex_simpleStatechart_init.Complex_simpleStatechart_init init(
			nIn=0,
			use_outPort=false,
			nSuspend=0,
			initialStep=true
	...
	algorithm
			...
			// transition actions
			when pre(init.region_1_Region1.transition_State_B_2_State_B.fire) then
