/** \mainpage Documentation of the generated Code for ECU: brick201
...
DeclareTask(Task_communicatorComponent);
DeclareTask(Task_loadControllerComponent);
...
TASK(Task_Main){
	...
	//Activate a task per component instance
		ActivateTask(Task_communicatorComponent);
		ActivateTask(Task_loadControllerComponent);
...
TASK(Task_communicatorComponent){
	CommunicatorComponent_processStep(mw->communicatorComponent);
	TerminateTask();
}	
TASK(Task_loadControllerComponent){
	LoadControllerComponent_processStep(mw->loadControllerComponent);
	TerminateTask();
}	
