...
bool_t Middleware_destroy(void){
	...
	CommunicatorComponent_destroy(mw->communicatorComponent);
	LoadControllerComponent_destroy(mw->loadControllerComponent);
...
bool_t MW_sendMessage(PortID targetPort, MessageID id, void *msg){
	...
	switch(id){
	case MESSAGE_TOUCHSENSOR_TOUCHPORT_MESSAGEAPI_MESSAGETYPE_REPOSITORYMESSAGE:
			nwMsg->_mumlMsg_len = TouchSensor_TouchPort_messageapi_messageType_RepositoryMessage_write_delimited_to((TouchSensor_TouchPort_messageapi_messageType_RepositoryMessage*)msg, nwMsg->_mumlMsg, 0);
			break;
		default:
