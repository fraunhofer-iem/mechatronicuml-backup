/** \mainpage Documentation of the generated Code for ECU: raspberryPi01
...
DeclareTask(Task_pickUpBotInstanceComponent);
...
TASK(Task_Main){
	...
	//Activate a task per component instance
		ActivateTask(Task_pickUpBotInstanceComponent);
...
TASK(Task_pickUpBotInstanceComponent){
	PickUpBotComponent_processStep(mw->pickUpBotInstanceComponent);
	TerminateTask();
}	
