...
bool_t Middleware_destroy(void){
	...
	DriverComponent_destroy(mw->driverComponent);
	MainControllerComponent_destroy(mw->mainControllerComponent);
...
bool_t MW_sendMessage(PortID targetPort, MessageID id, void *msg){
	...
	switch(id){
	case MESSAGE_COLORSENSOR_COLORPORT_MESSAGEAPI_MESSAGETYPE_REPOSITORYMESSAGE:
			nwMsg->_mumlMsg_len = ColorSensor_ColorPort_messageapi_messageType_RepositoryMessage_write_delimited_to((ColorSensor_ColorPort_messageapi_messageType_RepositoryMessage*)msg, nwMsg->_mumlMsg, 0);
			break;
	case MESSAGE_MOTOR_MOTORPORT_MESSAGEAPI_MESSAGETYPE_REPOSITORYMESSAGE:
			nwMsg->_mumlMsg_len = Motor_MotorPort_messageapi_messageType_RepositoryMessage_write_delimited_to((Motor_MotorPort_messageapi_messageType_RepositoryMessage*)msg, nwMsg->_mumlMsg, 0);
			break;
	case MESSAGE_LINELEADERSENSOR_LINELEADERPORT_MESSAGEAPI_MESSAGETYPE_REPOSITORYMESSAGE:
			nwMsg->_mumlMsg_len = LineLeaderSensor_LineLeaderPort_messageapi_messageType_RepositoryMessage_write_delimited_to((LineLeaderSensor_LineLeaderPort_messageapi_messageType_RepositoryMessage*)msg, nwMsg->_mumlMsg, 0);
			break;
		default:
