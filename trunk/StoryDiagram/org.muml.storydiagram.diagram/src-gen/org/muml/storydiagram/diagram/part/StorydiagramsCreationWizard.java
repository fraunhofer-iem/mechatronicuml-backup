...
public class StorydiagramsCreationWizard extends Wizard implements INewWizard {
	...
	public void init(IWorkbench workbench, IStructuredSelection selection) {
		...
		setDefaultPageImageDescriptor(
				StorydiagramsDiagramEditorPlugin.getBundledImageDescriptor("icons/wizban/NewActivitiesWizard.gif")); //$NON-NLS-1$
	...
	public void addPages() {
		diagramModelFilePage = new StorydiagramsCreationWizardPage("DiagramModelFile", getSelection(), //$NON-NLS-1$
				"storydiagrams_diagram"); //$NON-NLS-1$
		...
		domainModelFilePage = new StorydiagramsCreationWizardPage("DomainModelFile", getSelection(), "fujaba") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length() - ".storydiagrams_diagram".length()); //$NON-NLS-1$
	...
	public boolean performFinish() {
		...
		try {
			...
			if (e.getTargetException() instanceof CoreException) {
				...
				StorydiagramsDiagramEditorPlugin.getInstance().logError("Error creating diagram", //$NON-NLS-1$
