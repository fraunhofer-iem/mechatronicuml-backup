...
public class MumlNewDiagramFileWizard extends Wizard {
	...
	public MumlNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot, TransactionalEditingDomain editingDomain) {
		...
		myFileCreationPage.setDescription(NLS.bind(Messages.MumlNewDiagramFileWizard_CreationPageDescription,
				MessageInterfaceDiagramEditPart.MODEL_ID));
		IPath filePath;
		...
		myFileCreationPage
				.setFileName(MumlDiagramEditorUtil.getUniqueFileName(filePath, fileName, "messagetype_diagram")); //$NON-NLS-1$
	...
	public boolean performFinish() {
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
				...
				if (diagramVID != MessageInterfaceDiagramEditPart.VISUAL_ID) {
		...
		try {
			...
			MessageTypeDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		} catch (PartInitException ex) {
			MessageTypeDiagramEditorPlugin.getInstance().logError("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
