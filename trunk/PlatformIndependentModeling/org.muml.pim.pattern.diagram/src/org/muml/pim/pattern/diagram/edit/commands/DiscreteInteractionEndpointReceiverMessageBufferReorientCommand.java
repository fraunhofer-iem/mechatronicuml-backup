...
public class DiscreteInteractionEndpointReceiverMessageBufferReorientCommand extends EditElementCommand {
	...
	*/
	protected boolean canReorientSource() {
		...
		if (!MumlBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistDiscreteInteractionEndpointReceiverMessageBuffer_4003(getNewSource(), getOldTarget(),
						sourceView, targetView)) {
			String errorMessage = MumlBaseItemSemanticEditPolicy.getLinkConstraints()
					.getErrorDiscreteInteractionEndpointReceiverMessageBuffer_4003(getNewSource(), getOldTarget(),
							sourceView, targetView);
	...
	*/
	protected boolean canReorientTarget() {
		...
		View targetView = ConnectionConfigureHelperGraphicalNodeEditPolicy.getTargetView(getRequest());
		if (!MumlBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistDiscreteInteractionEndpointReceiverMessageBuffer_4003(getOldSource(), getNewTarget(),
						sourceView, targetView)) {
			String errorMessage = MumlBaseItemSemanticEditPolicy.getLinkConstraints()
					.getErrorDiscreteInteractionEndpointReceiverMessageBuffer_4003(getOldSource(), getNewTarget(),
							sourceView, targetView);
