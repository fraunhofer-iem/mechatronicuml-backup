...
public class VerifyForResultsJob extends Job {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		PropertyResultRepository resultRepository = null;
		try {
			int numExecutions = 4;
			
			SubMonitor subMonitor = SubMonitor.convert(monitor, this.getName(), 100 * numExecutions);
			...
			for (int i = 0; i < numExecutions; i++) {
				...
				resultRepository = (PropertyResultRepository) resultExtent.getContents().get(0);
			}
			
			//Show results
			subMonitor.subTask("Showing Results");
			propertyResultAcceptor.acceptResult(resultRepository);
