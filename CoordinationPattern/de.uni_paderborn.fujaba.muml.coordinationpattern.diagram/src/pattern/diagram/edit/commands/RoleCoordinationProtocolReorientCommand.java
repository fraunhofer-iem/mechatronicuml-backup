...
public class RoleCoordinationProtocolReorientCommand extends EditElementCommand {
	...
	protected boolean canReorientSource() {
		...
		if (!Pattern2BaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistRoleCoordinationProtocol_4001(getNewSource(),
						getOldTarget(), sourceView, targetView)) {
			String errorMessage = Pattern2BaseItemSemanticEditPolicy
					.getLinkConstraints()
					.getErrorRoleCoordinationProtocol_4001(getNewSource(),
							getOldTarget(), sourceView, targetView);
	...
	protected boolean canReorientTarget() {
		...
		View targetView = de.uni_paderborn.fujaba.common.edit.policies.node.ConnectionConfigureHelperGraphicalNodeEditPolicy
				.getTargetView(getRequest());
		if (!Pattern2BaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistRoleCoordinationProtocol_4001(getOldSource(),
						getNewTarget(), sourceView, targetView)) {
			String errorMessage = Pattern2BaseItemSemanticEditPolicy
					.getLinkConstraints()
					.getErrorRoleCoordinationProtocol_4001(getOldSource(),
							getNewTarget(), sourceView, targetView);
			de.uni_paderborn.fujaba.common.edit.policies.ErrorFeedbackEditPolicy
