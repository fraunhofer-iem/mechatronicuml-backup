...
public class MumlOCLFactory {
	...
	protected MumlOCLFactory() {
		this.expressions = new MumlAbstractExpression[28];
		this.expressionBodies = new String[] { "\'\'", //$NON-NLS-1$
				...
				"\'Discard: \'.concat(\n\tif (self.bufferOverflowAvoidanceStrategy = BufferOverflowAvoidanceStrategy::DISCARD_INCOMING_MESSAGE) then\n\t\t\'Incoming Msgs\'\n\telse\n\t\tif (self.bufferOverflowAvoidanceStrategy = BufferOverflowAvoidanceStrategy::DISCARD_OLDEST_MESSAGE_IN_BUFFER) then\n\t\t\t\'Oldest Msgs\'\n\t\telse\n\t\t\tif (self.bufferOverflowAvoidanceStrategy = BufferOverflowAvoidanceStrategy::NEVER_HAPPENS) then\n\t\t\t\t\'Never Happens\'\n\t\t\telse\n\t\t\t\t\'<Unknown>\'\n\t\t\tendif\n\t\tendif\n\tendif\n)", //$NON-NLS-1$
