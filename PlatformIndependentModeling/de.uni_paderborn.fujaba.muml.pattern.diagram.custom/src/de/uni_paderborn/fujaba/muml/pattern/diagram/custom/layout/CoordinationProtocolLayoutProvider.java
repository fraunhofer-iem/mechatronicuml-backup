package de.uni_paderborn.fujaba.muml.pattern.diagram.custom.layout;
	...
	public boolean provides(IOperation operation) {
		if (operation instanceof ILayoutNodeOperation) {
			...
			if (nodes.hasNext()) {
				...
				if (container == null
						|| !(container.getType().equals("Coordination Pattern"))) //$NON-NLS-1$
					return false;
			}
		} else {
			return false;
...
ListIterator li = layoutNodes.listIterator();
				...
				while(li.hasNext())
				{
					...
					if(semanticElement instanceof MessageBuffer)
					{
						...
						if(mb.getDiscreteInteractionEndpoint() == nodes.get(1).getNode().getElement())
						{
							leftMessageBuffer.add(lnode);
						}
						else if (mb.getDiscreteInteractionEndpoint()== nodes.get(2).getNode().getElement())
						{
                ...
                if(quosaNode !=null)
                {                    
                    Bounds bounds = (Bounds)quosaNode.getNode().getLayoutConstraint();
                ...
                {
                	  Bounds bounds = (Bounds)lNode.getNode().getLayoutConstraint();
                      bounds.setX(x[0] - lNode.getWidth()/2- currentWidthOffset);
                ...
                {
                	  Bounds bounds = (Bounds)lNode.getNode().getLayoutConstraint();
                      bounds.setX(x[1] -lNode.getWidth()/2 +currentWidthOffset);
