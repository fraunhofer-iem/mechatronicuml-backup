...
public class MumlOCLFactory {
	...
	protected MumlOCLFactory() {
		...
		this.expressionBodies = new String[] { "1", //$NON-NLS-1$
				...
				"-- Only PortParts may be connected using an AssemblyConnector\nself.oclIsKindOf(pim::component::PortPart)", //$NON-NLS-1$
				"-- DEACTIVATED FOR #941:\n-- Assembly must only connect PortParts at different ComponentParts of the same type\ntrue\n--self.oclIsKindOf(pim::component::PortPart) and\n--not oppositeEnd.oclAsType(pim::component::PortPart).componentPart.parentComponent.componentKind.oclIsUndefined() and\n--not self.oclAsType(pim::component::PortPart).componentPart.parentComponent.oclIsUndefined() and\n--oppositeEnd.oclAsType(pim::component::PortPart).componentPart <> self.oclAsType(pim::component::PortPart).componentPart and\n--oppositeEnd.oclAsType(pim::component::PortPart).componentPart.parentComponent = self.oclAsType(pim::component::PortPart).componentPart.parentComponent", //$NON-NLS-1$
				"-- Only Ports or PortParts may be the source of a DelegationConnector\ntrue\n-- or self.oclIsKindOf(pim::component::PortPart) or self.oclIsKindOf(pim::component::Port)", //$NON-NLS-1$
				"-- DelegationConnector must only connect Port at StructuredComponent and PortPart at embedded ComponentPart\r\nif self.oclIsKindOf(pim::component::PortPart) or  self.oclIsKindOf(pim::component::Port) then true\r\nelse false\r\nendif\r\n--let portPart : pim::component::PortPart = if self.oclIsKindOf(pim::component::PortPart) then self else if oppositeEnd.oclIsKindOf(pim::component::PortPart) then oppositeEnd else null endif endif.oclAsType(pim::component::PortPart) in\r\n--let portType : pim::component::Port = if self.oclIsKindOf(pim::component::Port) then self else if oppositeEnd.oclIsKindOf(pim::component::Port) then oppositeEnd else null endif endif.oclAsType(pim::component::Port) in\r\n--not portPart.oclIsUndefined() and\r\n--not portType.oclIsUndefined() and\r\n--not portPart.componentPart.parentComponent.oclIsUndefined() and\r\n--not portType.component.oclIsUndefined() and\r\n--portPart.componentPart.parentComponent = portType.component", //$NON-NLS-1$
