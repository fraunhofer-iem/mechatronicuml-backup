 ...
 * @model annotation="http://www.eclipse.org/emf/2002/Ecore constraints='OneInvarianPerClock OneInitialState NoOutgoingTransitionOfFinalState NoRegionsOfFinalState UniquePrioritiesOfOutgoingTransitions UniquePrioritiesOfRegions UniqueChannelNames UniqueRegionNames BoundOfInvariantGreaterOrEqualZero InvalidClockConstraintOperator'"
 *        annotation="http://www.eclipse.org/emf/2002/Ecore/OCL OneInvarianPerClock='self.invariants->isUnique(clock)' OneInitialState='not self.statechart.vertices->select(x | x.oclIsKindOf(State)).oclAsType(State)->select(s |  s.initial)->isEmpty()' NoOutgoingTransitionOfFinalState='self.final implies self.outgoingTransitions->isEmpty()' NoRegionsOfFinalState='self.final implies self.regions->isEmpty()' UniquePrioritiesOfOutgoingTransitions='self.outgoingTransitions->isUnique(priority) ' UniquePrioritiesOfRegions='self.regions->isUnique(priority)' UniqueChannelNames='self.channels->isUnique(name)' UniqueRegionNames='self.regions->isUnique(name)' BoundOfInvariantGreaterOrEqualZero='self.invariants->forAll(bound.value >= 0)' InvalidClockConstraintOperator='self.invariants->forAll(invariant | Set{ modeling::expressions::ComparingOperator::LESS, modeling::expressions::ComparingOperator::LESS_OR_EQUAL }->includes(invariant.operator))'"
 * @generated
 */
