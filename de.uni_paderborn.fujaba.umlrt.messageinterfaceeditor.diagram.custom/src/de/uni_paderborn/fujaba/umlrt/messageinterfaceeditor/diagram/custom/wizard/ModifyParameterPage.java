...
import de.uni_paderborn.fujaba.umlrt.messageinterfaceeditor.diagram.providers.MessageinterfaceElementTypes;
...
public class ModifyParameterPage extends WizardPage{
	...
	private void handleCreateParameterEvent(){
		...
		if(!parameterNameText.getText().equals("") && getSelectedDataType()!= null){
			CreateElementRequest request = new CreateElementRequest(((ModifyParameterWizard)getWizard()).getSelectedSyncChannel(),
					MessageinterfaceElementTypes.MessageInterface_2001);
	...
	private List<EDataType> getDataTypes(){
		...
		MessageInterfaceDiagram messageTypeDiagram = ((ModifyParameterWizard)getWizard()).getMessageTypeDiagram();
