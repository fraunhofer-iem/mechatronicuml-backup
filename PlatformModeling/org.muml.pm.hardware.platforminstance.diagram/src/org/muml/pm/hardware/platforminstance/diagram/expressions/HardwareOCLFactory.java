...
public class HardwareOCLFactory {
	...
	protected HardwareOCLFactory() {
		...
		this.expressionBodies = new String[] { "self.name", //$NON-NLS-1$
				...
				"\' \'", //$NON-NLS-1$
				"let isStructuredResource : Boolean = if self.resourceType.oclIsUndefined() then false else\n self.resourceType.oclIsTypeOf(hardware::hwresource::StructuredResource) endif in\nlet memory : Set(hardware::hwresourcepim::instance::MemoryResourceInstance) = if (isStructuredResource) then self.oclAsType(hardware::hwresourcepim::instance::StructuredResourceInstance).embeddedAtomicResourceInstances->select(r|r.oclIsKindOf(hardware::hwresourcepim::instance::MemoryResourceInstance) and not r.oclIsTypeOf(hardware::hwresourcepim::instance::CacheInstance)).oclAsType(hardware::hwresourcepim::instance::MemoryResourceInstance)->asOrderedSet()\nelse Set(hardware::hwresourcepim::instance::MemoryResourceInstance){}  endif in\nlet memoryString : String = if memory->isEmpty() then \'Memory:\' else memory->iterate(m; result : String = \'Memory:\' | result.concat(m.memorySize.toString().concat(\' \').concat(m.memoryType.toString()).concat(\', \'))) endif in\nmemoryString", //$NON-NLS-1$
				"let isStructuredResource : Boolean = if self.resourceType.oclIsUndefined() then false else\nself.resourceType.oclIsTypeOf(hardware::hwresource::StructuredResource) endif in\nlet processor : Set(hardware::hwresourcepim::instance::ComputingResourceInstance) = if (isStructuredResource) then self.oclAsType(hardware::hwresourcepim::instance::StructuredResourceInstance).embeddedAtomicResourceInstances->select(r|r.oclIsKindOf(hardware::hwresourcepim::instance::ComputingResourceInstance)).oclAsType(hardware::hwresourcepim::instance::ComputingResourceInstance)->asOrderedSet()\nelse Set(hardware::hwresourcepim::instance::ComputingResourceInstance){}  endif in\nlet processorString : String = if processor->isEmpty() then \'Processor:\' else processor->iterate(p; result : String = \'Processor:\' | result.concat(p.frequency.toString().concat(\', \'))) endif in\nprocessorString", //$NON-NLS-1$
				"\': \' + if self.resourceType.oclIsUndefined() then \'null\' else self.resourceType.name endif", //$NON-NLS-1$
				"\' \'", //$NON-NLS-1$
				"\': \' + if self.resourceType.oclIsUndefined() then \'null\' else self.resourceType.name endif", //$NON-NLS-1$
				"\' \'", //$NON-NLS-1$
