...
public class DiagramPartitioningUtil {
	...
	/**
	 * Opens the {@link } for a fiven {@link IFile}
	 * 
	 */
	 ...
	 */
	public static boolean closeSubdiagramEditors(EObject state) {
	...
	public static List<Diagram> getSubDiagramHierachy(Diagram diagram) {
		...
		result.add(diagram);
		while (diagram.getElement() instanceof State) {
			diagram = DiagramPartitioningUtil.getSubDiagram((State) diagram.getElement());
			result.add(diagram);
		}
		Collections.reverse(result);
		return result;
