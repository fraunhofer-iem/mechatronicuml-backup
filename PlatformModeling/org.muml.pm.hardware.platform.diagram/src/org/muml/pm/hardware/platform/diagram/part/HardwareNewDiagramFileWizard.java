...
public class HardwareNewDiagramFileWizard extends Wizard {
	...
	public HardwareNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot,
		...
		myFileCreationPage.setDescription(
				NLS.bind(Messages.HardwareNewDiagramFileWizard_CreationPageDescription, HWPlatformEditPart.MODEL_ID));
		IPath filePath;
		...
		myFileCreationPage
				.setFileName(HardwareDiagramEditorUtil.getUniqueFileName(filePath, fileName, "platform_diagram")); //$NON-NLS-1$
	...
	public boolean performFinish() {
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
				...
				if (diagramVID != HWPlatformEditPart.VISUAL_ID) {
					return CommandResult
							.newErrorCommandResult(Messages.HardwareNewDiagramFileWizard_IncorrectRootError);
				}
				Diagram diagram = ViewService.createDiagram(diagramRootElementSelectionPage.getModelElement(),
						HWPlatformEditPart.MODEL_ID, PlatformDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT);
		...
		try {
			...
			PlatformDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		} catch (PartInitException ex) {
			PlatformDiagramEditorPlugin.getInstance().logError("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
